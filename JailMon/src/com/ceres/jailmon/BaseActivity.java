...
public class BaseActivity extends Activity {
	...
	final static int API_POST_TX_RESULT_INFO_OK = 33;
	...
	final static int API_GET_ALL_POLICE_OK = 1004;
	...
	protected Handler m_basehandler = new Handler() {
		public void handleMessage(Message msg) {
			...
			case API_GET_SERVER_TIME_OK:
				...
				break;
			case API_POST_TX_RESULT_INFO_OK:
				onReceiveTXResult((TXResult)msg.obj);
				break;
				
			case API_GET_ALL_POLICE_OK:
				onReceiveAllPoliceResult((List<Police>)msg.obj);
				break;
			case API_GET_PATROL_HISTORY_OK:
				onReceivePatrolResult((List<PatrolHistory>)msg.obj);
				break;
			case API_POST_PATROL_OK:
				onReceivePostPatrolResult((PatrolResult)msg.obj);
				break;
			case API_GET_INDEX_DATA_OK:
				onReceiveIndexDataResult((List<RoomIndex>)msg.obj);
				break;
	...
	protected void onReceiveIndexDataResult(List<RoomIndex> indexDataList){
	...
	public void postTxData(final Handler handler, final String txID,
			final String txFlag) {
		new Thread() {
			public void run() {
				Message msg = new Message();
				try {
					TXResult info = m_AppContext.postTXResult(txID,
							txFlag);
					msg.what = API_POST_TX_RESULT_INFO_OK;
					msg.obj = info;
				} catch (AppException e) {
					e.printStackTrace();
					msg.what = API_GET_FAIL;
	...
	public void getAllPolice(final Handler handler) {
		new Thread() {
			public void run() {
				Message msg = new Message();

				try {
					List<Police> info = m_AppContext.getAllPolice();
					msg.what = API_GET_ALL_POLICE_OK;
					msg.obj = info;
				} catch (AppException e) {
					e.printStackTrace();
					msg.what = API_GET_FAIL;
	...
	public void getPatrolHistory(final Handler handler) {
		new Thread() {
			public void run() {
				Message msg = new Message();
				try {
					List<PatrolHistory> info = m_AppContext.getPatrolHistory();
					msg.what = API_GET_PATROL_HISTORY_OK;
					msg.obj = info;
				} catch (AppException e) {
					e.printStackTrace();
					msg.what = API_GET_FAIL;
	...
	public void getAllOutInfoPhoto(final Handler handler,
			final OutInfoList infolist) {
		new Thread() {
			public void run() {
				if (infolist == null)
					return;
	...
	public void getIndexData(final Handler handler) {
		new Thread() {
			public void run() {
				Message msg = new Message();
				List<RoomIndex> roomIndexList = new ArrayList<RoomIndex>();
				try {
					roomIndexList = m_AppContext.getIndexData();
					msg.what = API_GET_INDEX_DATA_OK;
					msg.obj = roomIndexList;
				} catch (AppException e) {
					e.printStackTrace();
					msg.what = API_GET_FAIL;
					msg.obj = e;
	...
	public void postPatrol(final Handler handler,final String pId) {
		new Thread() {
			public void run() {
				Message msg = new Message();
				try {
					PatrolResult info = m_AppContext.postPatrol(pId);
					msg.what = API_POST_PATROL_OK;
					msg.obj = info;
				} catch (AppException e) {
					e.printStackTrace();
					msg.what = API_GET_FAIL;
